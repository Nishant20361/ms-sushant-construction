...
export function HeroBanner({ settings, loading }: { settings?: SiteSettings | null; loading: boolean }) {
  ...
  useEffect(() => setImageFailed(false), [settings?.mobileHeroBannerUrl]);
  const image = imageFailed ? null : resolveImageUrl(settings?.mobileHeroBannerUrl, 900);
  return <View style={styles.hero}>{image ? <Image source={image} placeholder={{ blurhash: IMAGE_BLURHASH }} cachePolicy="memory-disk" contentFit="cover" transition={200} style={styles.decorativeImage} accessibilityLabel="Construction materials banner" onError={() => setImageFailed(true)} /> : null}<View style={styles.content}><Text style={styles.eyebrow}>BUILD WITH CONFIDENCE</Text><Text style={styles.title}>{settings?.mobileHeroTitle || "Quality Construction Materials"}</Text><Text style={styles.subtitle}>{settings?.mobileHeroSubtitle || "ACC Cement, Nuvoco Cement, Tata & Mongia steel rods, roofing sheets, waterproofing chemicals and more — at the best prices."}</Text><Link href="/(tabs)/products" asChild><Pressable accessibilityRole="button" style={({ pressed }) => [styles.cta, pressed && { opacity: 0.8 }]}><Text style={styles.ctaText}>Browse Products</Text><Text style={styles.ctaArrow}>→</Text></Pressable></Link>{loading ? <Text style={styles.connecting}>Connecting to business services…</Text> : null}</View></View>;
}
