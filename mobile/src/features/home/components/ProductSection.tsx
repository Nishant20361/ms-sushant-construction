...
export function ProductSection({ products, loading, error, onRetry, onAdd }: { products?: Product[]; loading: boolean; error: boolean; onRetry: () => void; onAdd: (product: Product) => void }) {
  return <View style={styles.section}><SectionHeader title="Latest products" subtitle="Recently added and currently available" actionLabel="View all" href="/(tabs)/products" />
    ...
    {products?.length ? <View style={styles.grid}>{products.map((product) => <ProductCard key={product.id} product={product} onAdd={() => onAdd(product)} />)}</View> : null}
  </View>;
}
