...
function CategorySkeleton({ width }: { width: number }) {
  return <View style={[styles.skeleton, { width, marginRight: CATEGORY_GAP }]}><View style={styles.skeletonIcon} /><View style={styles.skeletonLine} /></View>;
}
...
export function CategorySection({ categories, loading, error, onRetry }: CategorySectionProps) {
  ...
  return <View style={styles.section}>
    ...
    <View onLayout={(event) => setViewportWidth(Math.floor(event.nativeEvent.layout.width))} style={styles.listViewport}>
...
const styles = StyleSheet.create({
  ...
  list: { paddingTop: 14, paddingRight: 8 },
  card: { height: 118, flexGrow: 0, flexShrink: 0, alignItems: "center", paddingTop: 7, borderWidth: 1, borderColor: "#D5E9E4", borderRadius: 16, backgroundColor: theme.colors.surface },
  ...
  nameRegion: { width: "100%", height: 30, marginTop: 4, paddingHorizontal: 1, alignItems: "center", justifyContent: "center", overflow: "hidden" },
  ...
  count: { width: "100%", height: 15, marginTop: 2, textAlign: "center", fontSize: 9.5, lineHeight: 15, color: theme.colors.muted, includeFontPadding: false },
  skeleton: { height: 118, flexShrink: 0, alignItems: "center", paddingTop: 7, borderRadius: 16, backgroundColor: theme.colors.surface },
