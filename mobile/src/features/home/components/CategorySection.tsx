...
export function CategorySection({ categories, loading, error, onRetry }: { categories?: Category[]; loading: boolean; error: boolean; onRetry: () => void }) {
  ...
  const availableWidth = screenWidth - HOME_HORIZONTAL_PADDING * 2;
  const cardWidth = Math.round((availableWidth - CATEGORY_GAP * (VISIBLE_CATEGORY_COUNT - 1)) / VISIBLE_CATEGORY_COUNT);
...
const styles = StyleSheet.create({ section: { marginTop: 28 }, list: { gap: CATEGORY_GAP, paddingTop: 14, paddingRight: 16 }, card: { height: 120, flexGrow: 0, flexShrink: 0, alignItems: "center", paddingTop: 7, borderWidth: 1, borderColor: "#D5E9E4", borderRadius: 18, backgroundColor: theme.colors.surface, ...theme.shadow }, pressed: { opacity: 0.72, transform: [{ scale: 0.98 }] }, iconCircle: { width: 48, height: 48, overflow: "hidden", alignItems: "center", justifyContent: "center", borderRadius: 14, backgroundColor: "#CCFBF1" }, categoryImage: { width: 48, height: 48 }, icon: { fontSize: 22, color: theme.colors.primaryDark }, nameRegion: { alignSelf: "stretch", height: 30, marginTop: 4, paddingHorizontal: 1, justifyContent: "center", overflow: "hidden" }, name: { textAlign: "center", fontSize: 10, lineHeight: 15, fontWeight: "700", color: theme.colors.text }, count: { alignSelf: "stretch", height: 15, marginTop: 2, textAlign: "center", fontSize: 9.5, lineHeight: 15, color: theme.colors.muted }, skeleton: { height: 120, alignItems: "center", paddingTop: 7, borderRadius: 18, backgroundColor: theme.colors.surface }, skeletonIcon: { width: 48, height: 48, borderRadius: 14, backgroundColor: theme.colors.border }, skeletonLine: { width: 56, height: 10, marginTop: 10, borderRadius: 6, backgroundColor: theme.colors.border }, error: { minHeight: 72, marginTop: 14, justifyContent: "center", padding: 14, borderRadius: 12, backgroundColor: "#FEF2F2" }, errorTitle: { color: theme.colors.danger, fontWeight: "700" }, retry: { marginTop: 3, color: theme.colors.primary, fontWeight: "700" }, empty: { marginTop: 14, color: theme.colors.muted } });
